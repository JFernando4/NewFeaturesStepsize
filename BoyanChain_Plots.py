...
from src import get_average_and_standard_error, load_results, load_avg_learning_curve
...
def get_num_features(exp_type):
    ...
    if exp_type == 'add_bad_feats':
        return 4, 0, 4      # initial good features, added new features, added bad features
    elif exp_type == 'add_good_feats':
        return 4, 4, 0
    elif exp_type == 'continuously_add_bad':
        return 4, 0, 200
    elif exp_type == 'add_4_good_4_bad':
        return 4, 4, 4
    elif exp_type == 'add_4_good_20_bad':
        return 4, 4, 20
    elif exp_type == 'add_4_good_100_bad':
        return 4, 4, 100
    else:
        raise ValueError("{0} is not a valid method.".format(exp_type))


def sgd_plots(save_plots=False, fake_features=False, bin_size=5):
    assert NUM_SAMPLES % bin_size == 0
    Xaxis = (np.arange(NUM_SAMPLES // bin_size) + 1) * (100 * bin_size)

    method = 'sgd'

    if fake_features:
        task_name = 'boyan_chain_task_add_fake_features'
        experiment_names = ['add_bad_feats', 'continuously_add_bad']
    else:
        task_name = 'boyan_chain_task_add_features'
        experiment_names = ['add_good_feats', 'add_bad_feats', 'continuously_add_bad']

    results_dir = os.path.join(os.getcwd(), 'results', task_name, 'feature_noise0.1')
    ylim = (0.0, 0.25)
    ...
    for exp_name in experiment_names:
        ...
        for i, n in enumerate(names):
            avg, ste = get_average_and_standard_error(data_array=results_dict[n]['sample_msve'], bin_size=bin_size,
        ...
        avg, ste = load_avg_learning_curve(results_dirpath=os.path.join(results_dir, 'rescaled_sgd', exp_name),
                                           ...
                                           num_samples=NUM_SAMPLES, secondary_results_name='sample_msve')
        plt.plot(Xaxis, avg, color=RESCALED_SGD_COLORS['solid'], linestyle='dotted', label='rescaled_sgd')
        ...
        plt.ylabel("Sample Mean\nSquared Error", fontsize=18, rotation=0, labelpad=75)
    ...
    ########################################
    experiment_names = ['add_4_good_4_bad', 'add_4_good_20_bad', 'add_4_good_100_bad']
    ...
    for exp_name in experiment_names:
        ...
        for j in range(n):
            ...
            for k in range(n):
                name_index = j * n + k
                avg, ste = get_average_and_standard_error(data_array=results_dict[names[name_index]]['sample_msve'],
...
def stepsize_methods_plots(fake_features=False, save_plots=False, plot_learning_curves=False, plot_stepsizes=False,
    ...
    experiment_names = ['add_bad_feats', 'add_4_good_4_bad', 'add_4_good_20_bad',
    ...
    if plot_stepsizes:
        ...
        ylim = {
            'boyan_chain_task_add_features': {'add_good_feats': (0, 0.23), 'add_bad_feats': (0, 0.15),
                                              'add_4_good_4_bad': (0, 0.20), 'add_4_good_20_bad': (0, 0.15),
                                              'add_4_good_100_bad': (0, 0.15), 'continuously_add_bad': (0, 0.15)},
            'boyan_chain_task_add_fake_features': {'add_bad_feats': (0, 0.15), 'add_4_good_4_bad': (0, 0.23),
                                                   'add_4_good_20_bad': (0, 0.23), 'add_4_good_100_bad': (0, 0.20),
                                                   'continuously_add_bad': (0, 0.15)}
        }
        ...
        for exp_name in experiment_names:
            for i, m in enumerate(methods):
                ...
                if exp_name != 'continuously_add_bad':
                    plt.vlines(x=MIDPOINT, ymin=0, ymax=100, colors='#BD2A4E', linestyles='dashed')
                plt.ylim(ylim[task_name][exp_name])
                if save_plots:
